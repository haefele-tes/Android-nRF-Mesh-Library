...
import no.nordicsemi.android.meshprovisioner.IvIndex;
...
public class SecureNetworkBeacon extends MeshBeacon {
    ...
    public SecureNetworkBeacon(@NonNull final byte[] beaconData) {
        ...
        final boolean isIvUpdateActive = ((flags & 0x02) >> 1) == 1;
    ...
    public boolean authenticate(final byte[] n) {
        final ByteBuffer byteBuffer = ByteBuffer.wrap(beaconData);
        byteBuffer.position(1);
        final byte[] flagsNetworkIdAndIVIndex = new byte[13];
        byteBuffer.get(flagsNetworkIdAndIVIndex, 0, 13);
        return SecureUtils.authenticateSecureNetBeacon(flagsNetworkIdAndIVIndex, authenticationValue, n);
    }
